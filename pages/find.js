      ...
      <div className="relative">
        ...
        <section className="relative w-[710px] flex-grow pt-14 px-6 mt-8">
          ...
          >
            ...
            <p
              className="px-4 py-2 border rounded-full rsor-pointer hover:shadow-lg active:scale-95 active:bg-gray-100 transition transform duration-100 ease-out"
...
export async function getServerSideProps({ query }) {
  ...
  const searchResults = await fetch(
    `http://localhost:3331/Imoveis?Provincia=${prov}&Modalidade=${mod}`
  ).then((res) => res.json());
